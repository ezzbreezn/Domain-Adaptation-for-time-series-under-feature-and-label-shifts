...
class cross_domain_trainer(object):
   ...
   This class contain the main training functions for our AdAtime
    ...
    def train(self):
        ...
        for i in scenarios:
            ...
            for run_id in range(self.num_runs):  # specify number of consecutive runs
                ...
                for epoch in range(1, self.hparams["num_epochs"] + 1):
                    ...
                    for step, ((src_x, src_y), (trg_x, trg_index)) in joint_loaders:
                        src_x, src_y, trg_x, trg_index = src_x.float().to(self.device), src_y.long().to(self.device), \
                                              trg_x.float().to(self.device), trg_index.long().to(self.device)

                        if self.da_method in ["DANN", "CoDATS", "AdaMatch", 'DANCE', 'OVANet']:
                            losses = algorithm.update(src_x, src_y, trg_x, trg_index, step, epoch, len_dataloader)
                        else:
        ...
        log = {'scenario': i,'run_id':run_id,'accuracy':mean_acc,'f1':mean_f1}
        df_a = df_a.append(log, ignore_index=True)
        print(df_a)
    ...
    def avg_result(self, df, name):
        ...
        mean_acc_value = mean_acc['accuracy'].mean()
        std_acc_value = std_acc['accuracy'].std()
        mean_f1_value = mean_f1['f1'].mean()
        std_f1_value = std_f1['f1'].std()
        return mean_acc_value, std_acc_value, mean_f1_value, std_f1_value
